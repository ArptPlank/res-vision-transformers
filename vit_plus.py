...
class PatchEmbed(nn.Module):
    def __init__(self, input_shape=[224, 224], patch_size=16, in_chans=3, num_features=768, norm_layer=None,
        ...
        self.resblock= []
        for i in range(12):
            self.resblock.append(BasicBlock(num_features,num_features))
...
def vit_plus_(input_shape=[224, 224], pretrained=False, num_classes=1000):
    model = VisionTransformer(input_shape)
    if pretrained:
        model.load_state_dict(torch.load("./vit-patch_16.pth"),strict=False)
